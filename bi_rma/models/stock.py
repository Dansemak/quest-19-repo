...
class RmaStockPicking(models.Model):
    ...
    def button_validate(self):
        ...
        for picking in self:
            if picking.picking_type_code == "incoming" and picking.rma_id:
                ...
                if not rma_claim_obj.search_count(
                    [("stock_picking_id", "=", picking.rma_id.delivery_order.id)]
                ):
                ...
                if picking.state == "done":
                    invoice_ids = self.env["account.move"].search(
                        [
                            ("picking_id", "=", picking.rma_id.delivery_order.id),
    ...
    def create_credit_note_for_rma_in(self):
        ...
        fiscal_position_id = (
            ...
            .with_context({"rma_id": self.rma_id})
        ...
        for line in self.move_ids:
            ...
            #         lambda l: l.rma_line_id and l.rma_line_id in self.rma_id.rma_line_ids):
        ...
        if service_prds:
            for service_prd in service_prds.filtered(
                lambda service_prd: service_prd.delivery_qty > 0
            ):
                vals = (
                    0,
                    0,
        ...
        move_ids = self.env["account.move"].search(
            [
                ("picking_id", "=", self.rma_id.delivery_order.id),
                ("move_type", "=", "out_invoice"),
                ("state", "!=", "cancel"),
            ],
            order="id DESC",
        ...
        move_vals = {
            ...
            "ref": self.rma_id.sale_order.client_order_ref,
            "sale_id": self.rma_id.sale_order.id,
            "invoice_date": self.date_deadline or False,
            "fiscal_position_id": fiscal_position_id.id
            if fiscal_position_id
            else False,  # ✅ FIXED HERE
            "invoice_line_ids": invoice_line_list,
