...
class SaleOrderLine(models.Model):
    ...
    seq_no = fields.Integer(
        string="#", compute="_compute_line_number", readonly=True, store=True
    )
    tagging = fields.Char(
    ...
    def _compute_line_number(self):
        for line in self:
            if line.order_id:
                ...
                lines = line.order_id.order_line.filtered(
                    lambda i: not i.display_type
    ...
    def create(self, vals_list):
        if isinstance(vals_list, list):
            for vals in vals_list:
                if (
                    "string_availability_info" in vals
                    and vals["string_availability_info"]
                ):
                    vals["string_availability_info"] = (
                        vals["string_availability_info"].upper().strip()
                    )
                if "tagging" in vals and vals["tagging"]:
                    vals["tagging"] = vals["tagging"].strip()
        else:
            if (
                "string_availability_info" in vals_list
                and vals_list["string_availability_info"]
            ):
                vals_list["string_availability_info"] = (
                    vals_list["string_availability_info"].upper().strip()
                )
    ...
    def _onchange_string_availability_info(self):
        for record in self:
            if record.string_availability_info:
                record.string_availability_info = (
                    record.string_availability_info.upper().strip()
                )
