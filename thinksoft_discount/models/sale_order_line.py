...
class SaleOrderLine(models.Model):
    ...
    price_unit_discounted = fields.Monetary("Unit Price Discount")

    @api.depends('product_uom_qty', 'discount', 'price_unit', 'tax_ids')
    def _compute_amount(self):
        for line in self:
            if line.discount > 0.0:
                line.price_unit_discounted = round(line.price_unit * (1 - (line.discount / 100.0)), 2)
            else:
        ...
        try:
            for line, _, _ in originals:
                line.price_unit = line.price_unit_discounted
                line.discount = 0.0
